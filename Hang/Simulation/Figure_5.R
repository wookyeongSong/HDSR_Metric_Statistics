...
set.seed(2024)
n<-40000;alpha=0.05
Mean<-list();Cov<-list();a=0.5
Mean[[1]]<-c(-2,2);Mean[[2]]<-c(2,-2);
Cov[[1]]<-matrix(c(1,0.3,0.3,0.5),2,2);Cov[[2]]<-matrix(c(0.5,-0.3,-0.3,0.5),2,2)
Training_set<-list()
for (i in 1:n) {
  temp<-rbinom(1,1,a)+1
  Training_set[[i]]<-mvrnorm(1, Mean[[temp]],Cov[[temp]])
}
...
d_profile_b<-Dist_profile(bro_p , workgrid_p, Training_set,met_R2)
d_profile_o<-Dist_profile(org_p , workgrid_p, Training_set,met_R2)
...
b_cdf_smth<-locpoly(workgrid_p , d_profile_b, bandwidth=0.8, degree=1)
o_cdf_smth<-locpoly(workgrid_p , d_profile_o, bandwidth=0.8, degree=1)


c_cdf_smth$y[c_cdf_smth$y<0]<-0
m_cdf_smth$y[m_cdf_smth$y<0]<-0
f_cdf_smth$y[f_cdf_smth$y<0]<-0
b_cdf_smth$y[b_cdf_smth$y<0]<-0
o_cdf_smth$y[o_cdf_smth$y<0]<-0
o_cdf_smth$y[o_cdf_smth$y>1]<-1
...
p_m_combined<-ggplot(data=data.frame(x=f_cdf_smth$x,y=f_cdf_smth$y),aes(x=x,y=y))+
  ...
  geom_line(data=data.frame(x=b_cdf_smth$x,y=b_cdf_smth$y),aes(x=x,y=y),color = "brown",size=1.2)+
...
d_pdf_b<-diff(d_profile_b)
d_pdf_c<-c(0,d_pdf_c);d_pdf_m<-c(0,d_pdf_m);d_pdf_f<-c(0,d_pdf_f);d_pdf_b<-c(0,d_pdf_b)
c_pdf_smth<-locpoly(workgrid_p , d_pdf_c, bandwidth=1.5, degree=1)
m_pdf_smth<-locpoly(workgrid_p , d_pdf_m, bandwidth=1.5, degree=1)
f_pdf_smth<-locpoly(workgrid_p , d_pdf_f, bandwidth=1.5, degree=1)
b_pdf_smth<-locpoly(workgrid_p , d_pdf_b, bandwidth=1.5, degree=1)
...
b_pdf_smth$y<-b_pdf_smth$y/sum(b_pdf_smth$y)*length(b_pdf_smth$x)/10
f_pdf_smth$y[f_pdf_smth$y<0]<-0
b_pdf_smth$y[b_pdf_smth$y<0]<-0
m_pdf_smth$y[m_pdf_smth$y<0]<-0
c_pdf_smth$y[c_pdf_smth$y<0]<-0

p_b_combined<-ggplot(data=data.frame(x=f_pdf_smth$x,y=f_pdf_smth$y),aes(x=x,y=y))+
  geom_line(color = "blue",size=1.2)+  labs(x="r", y="")+
  geom_line(data=data.frame(x=m_pdf_smth$x,y=m_pdf_smth$y),aes(x=x,y=y),color = "purple",size=1.2,linetype="dashed")+
  geom_line(data=data.frame(x=c_pdf_smth$x,y=c_pdf_smth$y),aes(x=x,y=y),color = "red",size=1.2)+
  geom_line(data=data.frame(x=b_pdf_smth$x,y=b_pdf_smth$y),aes(x=x,y=y),color = "brown",size=1.2)+
  theme_minimal()+
  theme(
    axis.title.x = element_text(size = 14),  # Font size for x-axis label
    axis.title.y = element_text(size = 14),  # Font size for y-axis label
